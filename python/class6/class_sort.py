...
class Sort:
    ...
    def quick_sort(self,list1,left,high):
        l = left
        h = high
        base = list1[h]
        # 实现第一轮比较
        ...
        if h == high:
            ...
            # 使用递归，对右边剩下元素排序
            self.quick_sort(list1, h + 1, high)
        ...
        if len(list) < 2:
            ...
            for i in list:
                ...
                if i >= middle:
                    ...
                    left.append(i)
            # 使用迭代进行比较
            list = self.erfen(left) + [middle] + self.erfen(right)
            return list